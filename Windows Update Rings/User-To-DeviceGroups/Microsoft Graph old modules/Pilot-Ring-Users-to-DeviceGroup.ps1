...
$usergroup = Get-MgGroup -Filter "Displayname eq 'CWS-Windows_Personal_Pilot'"
$allusers = Get-MgGroupMember -GroupId $usergroup.Id  -All
$devicegroup = Get-MgGroup -Filter "Displayname eq 'CWS-Windows_Personal_Pilot_Devices'"
...
foreach ($ObjectIdUser in $ObjectIdUsers) {
    ...
    if (!$DeviceInfo) {
        "No device for user: $($ObjectIdUser.Name). Cannot add user to the group."
    }
    else {
        foreach ($Device in $DeviceInfo) {
            $DeviceID = Get-MgDevice -Filter "deviceId eq '$($Device.AzureActiveDirectoryDeviceId)'"
